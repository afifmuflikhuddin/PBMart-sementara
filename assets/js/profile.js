...
(function () {
  ...
  function fillStoreForm(user) {
    ...
    const waEl = $('pf-whatsapp');
    if (waEl) waEl.value = user.whatsapp || '';
  }
  ...
  function saveStore() {
    ...
    const whatsapp  = ($('pf-whatsapp')?.value.trim() || '').replace(/[^0-9]/g, '');
    ...
    const result = PM_AUTH.updateStoreInfo({ storeName, category, location, whatsapp });
    if (!result.ok) {
